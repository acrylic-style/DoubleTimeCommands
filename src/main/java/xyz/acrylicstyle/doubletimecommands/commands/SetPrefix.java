...
public class SetPrefix implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length == 0) {
			sender.sendMessage(ChatColor.RED + "You need 1 more argument at least! (new prefix) [player]");
			return true;
		}
		String uuid = null;
		String name = null;
		...
		try {
			ConfigProvider.setThenSave("players." + uuid + ".prefix", args[0], "DoubleTimeCommands");
		} catch (IOException | InvalidConfigurationException e) {
			e.printStackTrace();
			e.getCause().printStackTrace();
			sender.sendMessage(ChatColor.RED + "Error while saving prefix! Please try again later.");
			return true;
		}
		sender.sendMessage(ChatColor.GREEN + "Your prefix has been set to: \"" + ChatColor.translateAlternateColorCodes('&', args[0]) + ChatColor.RESET + ChatColor.GREEN + "\"");
		sender.sendMessage(ChatColor.GRAY + "To reset your prefix, type /resetprefix.");
